...
DEMO.init_demo = function (my_name) {
  ...
  DEMO.create_token(my_name, "presenter", function (response) {
	...
	stream.muteVideo(true);
       // add_stream_to_parrilla("test" + stream.getID());
      });
...
var add_stream_to_parrilla = function(divId) 
{

   setInterval(function(){
    // Añadimos a la parrilla
    if (divId != 'localVideo')
    {
	
	
        var streamId = divId.replace('test','');
	
        var stream = room.remoteStreams.get(streamId);
        var captura = stream.getVideoFrameURL();

	var imgElemento = document.getElementById('captura'+streamId);
	if(imgElemento)
	{
	...
	else
        {
          ...
          imgElemento.setAttribute("width","80px");
          imgElemento.setAttribute("height","60px");
          imgElemento.setAttribute("src", captura);
          imgElemento.setAttribute("id", 'captura' + streamId);
	...
	if (stream)
	{
   		if (entry.intersectionRatio == 0) 
		{
			console.log("Reduciendo video");
       			stream.muteVideo(true);
       			//stream.updateConfiguration({video:false}, function (result){ });
		...
		else if( entry.intersectionRatio == 1)
		{
			...
			stream.muteVideo(false);
                	//stream.updateConfiguration({video:true}, function (result){ });
		}
	}
  });
}
